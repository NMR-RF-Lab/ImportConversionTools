function procpar = readprocpar(file_loc)
            procdir = file_loc;
            
            % argument check
            [~,~,ext] = fileparts(procdir);
            if strcmp(ext,'.fid') ~= 1
                errordlg('Please choose a .fid file type','File Error');
            else
                ...
                if (fprocpar == -1)
                    fprintf('(readprocpar) ERROR: Could not open %s\n',[procdir 'procpar']);
                    return
                end
                ...
                if (isfield(procpar,'petable'))
                    ...
                    if (strncmp(token,'epi',3))
                        ...
                        while((j < length(token)) && (isempty(strfind('0123456789',token(j))))),j=j+1; %#ok<*STREMP>
                        end
                        % then skip the numbers
                        while((j < length(token)) && (~isempty(strfind('0123456789',token(j))))),j=j+1;
                        end
                        ...
                        % find end of numbers
                        k = j;
                        while((k < length(token)) && (~isempty(strfind('0123456789',token(k))))),k=k+1;
                        end
